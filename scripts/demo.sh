...
echo "   3. fhir-ig-testgen run --package package/package.tgz --config config.toml"
echo ""
echo "   Save detailed results to JSON:"
echo "   fhir-ig-testgen run --package package/package.tgz --config config.toml -o results.json"
echo ""
echo "   Preview all test URLs without executing:"
echo "   fhir-ig-testgen run --package package/package.tgz --config config.toml --dry-run"
echo ""
echo "🎉 Demo complete!"